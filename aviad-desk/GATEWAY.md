...
#### ד.5 "פרסמתי" - הנוסח הסופי של פוסט

טריגר: הודעה שמתחילה ב-"פרסמתי" / "פורסם" / "הנוסח הסופי", **או** reply על הודעת "פוסט מוכן".
זו הסגירה של לולאת הלמידה (`routines/amplifier.md` שלב 8) - בלעדיה המערכת מנחשת את הקול שלו לנצח.

1. זהה את הטיוטה: מתוך ה-reply (שורת `קובץ: {DATE}-{slug}` בהודעה המקורית), או הטיוטה
   האחרונה ב-`amplify/draft-*.md`. **`{DATE}` הוא תאריך הטיוטה, לא היום** - על השם הזה
   `diff_learn.py` מזווג. לא ניתן לזהות - `{היום}-unmatched`, בלי front-matter
2. כתוב ל-`inbox/published/{DATE}-{slug}.md`: שורות front-matter
   `---` / `draft: aviad-desk/amplify/draft-{DATE}-{slug}.md` / `---`, ואחריהן הטקסט **כלשונו, בלי עריכה**
3. ענה: "נשמר. ההבדל מהטיוטה ייכנס ללמידה של הפוסט הבא"
